  ...
  useEffect(() => {
    ...
    async function loadPatient() {
      ...
      try {
        ...
        if (cancelled) return
        setError(fetchError.message || t('unable_load_patient_record'))
        setPatient(null)
        setRecords([])
        setPrescriptions([])
        setReports([])
    ...
    return (
      <div className="space-y-6 page-enter pb-10">
        <div className="rounded-[2rem] border border-amber-200 bg-amber-50 p-6 text-amber-900 dark:border-amber-500/30 dark:bg-amber-500/10 dark:text-amber-200">
          ...
          <p className="mt-2 text-sm">{t('patient_access_invalid_link')}</p>
        ...
        <div className="grid gap-6 xl:grid-cols-[1fr_320px]">
          <div className="space-y-6">
            ...
            <div className="rounded-[2rem] border border-slate-200 bg-white p-6 dark:border-slate-700 dark:bg-slate-800">
              <div className="flex items-center gap-2 text-lg font-semibold text-slate-900 dark:text-slate-100"><FileText className="h-5 w-5 text-indigo-600" />{t('recent_records')}</div>
              <div className="mt-5 space-y-4">
                {records.length > 0 ? records.map(record => (
                  <div key={record.id} className="rounded-2xl border border-slate-200 p-4 dark:border-slate-700">
                    <div className="flex flex-wrap items-start justify-between gap-3">
                      <div>
                        <h3 className="font-semibold text-slate-900 dark:text-slate-100">{getDiagnosisStatusKey(record.diagnosis) ? t(getDiagnosisStatusKey(record.diagnosis)) : record.diagnosis}</h3>
                      ...
                      <span className="rounded-full bg-slate-100 px-3 py-1 text-xs font-medium text-slate-600 dark:bg-slate-700 dark:text-slate-300">{record.icd10_code || t('not_available')}</span>
                    </div>
            ...
            <div className="rounded-[2rem] border border-slate-200 bg-white p-6 dark:border-slate-700 dark:bg-slate-800">
              <div className="flex items-center gap-2 text-lg font-semibold text-slate-900 dark:text-slate-100"><Pill className="h-5 w-5 text-indigo-600" />{t('prescriptions')}</div>
              <div className="mt-5 grid gap-3 md:grid-cols-2">
                {prescriptions.length > 0 ? prescriptions.map(prescription => (
                  <div key={prescription.id} className="rounded-2xl border border-slate-200 p-4 dark:border-slate-700">
                    ...
                    <div className="mt-1 text-xs text-slate-400">{t('days_count', { count: prescription.duration_days || 0 })}</div>
                  </div>
                )) : (
          ...
          <aside className="space-y-6">
            <div className="rounded-[2rem] border border-slate-200 bg-white p-6 dark:border-slate-700 dark:bg-slate-800">
              <div className="flex items-center gap-2 text-lg font-semibold text-slate-900 dark:text-slate-100"><FlaskConical className="h-5 w-5 text-indigo-600" />{t('reports')}</div>
