    ...
    return (
      <div className="space-y-6 page-enter">
        <div className="rounded-[2rem] border border-amber-200 bg-amber-50 p-6 text-amber-900 dark:border-amber-500/30 dark:bg-amber-500/10 dark:text-amber-200">
          <h1 className="text-lg font-semibold">Patient record not found</h1>
        ...
        <div className="flex flex-col sm:flex-row items-start sm:items-center gap-6">
          ...
          <div className="flex-1">
            <div className="flex items-center gap-3 flex-wrap">
              ...
              <span className={`px-3 py-1 rounded-full text-xs font-medium ${RISK_BADGE_CLASSES[patient.risk_level] || 'bg-slate-100 text-slate-700 dark:bg-slate-700 dark:text-slate-200'}`}>{patient.risk_level} Risk</span>
            </div>
                ...
                <div key={record.id} className="border border-slate-100 dark:border-slate-700 rounded-xl p-4">
                  <div className="flex items-center justify-between mb-2">
                    <div>
                      ...
                      <p className="text-sm text-slate-500 dark:text-slate-400">{formatDate(record.visit_date)} · {record.doctors?.users?.full_name || 'Doctor'}</p>
                    </div>
                    <span className="font-mono text-xs bg-slate-100 dark:bg-slate-700 text-slate-600 dark:text-slate-400 px-2 py-0.5 rounded-lg">{record.icd10_code || 'N/A'}</span>
                  </div>
                  <p className="text-sm text-slate-600 dark:text-slate-300">{record.notes || 'No notes available'}</p>
                </div>
              ))}
              {records.length === 0 && <p className="text-sm text-slate-500 dark:text-slate-400">No data available</p>}
            </div>
          )}
                ...
                {filteredPrescriptions.map(prescription => (
                  <div key={prescription.id} className="border border-slate-100 dark:border-slate-700 rounded-xl p-4">
                    <div className="flex items-center justify-between">
                      ...
                      <span className={`text-xs px-2.5 py-0.5 rounded-full ${prescription.is_active === false ? 'bg-slate-100 text-slate-500 dark:bg-slate-700' : 'bg-green-100 text-green-700 dark:bg-green-900/30 dark:text-green-400'}`}>{prescription.is_active === false ? 'Inactive' : 'Active'}</span>
                    </div>
                    <p className="text-sm text-slate-500 dark:text-slate-400 mt-1">{prescription.dosage || '—'} · {prescription.frequency || '—'} · {prescription.duration_days || 0}d</p>
                  </div>
                ))}
                {filteredPrescriptions.length === 0 && <p className="text-sm text-slate-500 dark:text-slate-400">No data available</p>}
              </div>
            </div>
          )}
              ...
              {reports.map(report => (
                <a key={report.id} href={report.file_url} target="_blank" rel="noreferrer" className="flex items-center justify-between border border-slate-100 dark:border-slate-700 rounded-xl p-4">
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      <p className="text-sm font-medium text-slate-800 dark:text-slate-100">{report.report_type || 'Lab report'}</p>
                      <p className="text-xs text-slate-400">Uploaded {formatDate(report.uploaded_at)}</p>
                    </div>
                  </div>
            ...
            <div className="space-y-3">
              {records.map(record => (
                <div key={record.id} className="flex items-center justify-between border border-slate-100 dark:border-slate-700 rounded-xl p-4">
                  <div>
                    <p className="font-medium text-slate-800 dark:text-slate-100">{record.doctors?.hospital_name || 'Clinic'}</p>
                    <p className="text-sm text-slate-500 dark:text-slate-400">{formatDate(record.visit_date)} · {record.doctors?.users?.full_name || 'Doctor'}</p>
                  </div>
                  <span className="text-xs px-2.5 py-1 rounded-full bg-indigo-100 text-indigo-700 dark:bg-indigo-900/30 dark:text-indigo-300">Visit</span>
                </div>
              ))}
              {records.length === 0 && <p className="text-sm text-slate-500 dark:text-slate-400">No data available</p>}
            </div>
