...
export default function RiskTable() {
  ...
  useEffect(() => {
    ...
    async function loadWorkers() {
      ...
      try {
        ...
        if (!cancelled) {
          setWorkers([])
          setError(fetchError.message || 'Unable to load risk table data')
        }
      } finally {
  ...
  const COLS = [
    ...
    { key: 'last_checkup_date', label: 'Last Checkup Date' },
  ]
    ...
    <div className="space-y-6 page-enter">
      <div>
        ...
        <p className="text-slate-500 dark:text-slate-400 text-sm mt-1">
          {filtered.length.toLocaleString()} workers {totalPages > 0 ? `· Page ${page} of ${safeTotalPages}` : ''}
        </p>
      </div>
      ...
      <div className="bg-white dark:bg-slate-800 rounded-2xl shadow-sm border border-slate-100 dark:border-slate-700 p-4">
        <div className="flex flex-wrap gap-3">
          <div className="flex items-center gap-2 flex-1 min-w-48 border border-slate-200 dark:border-slate-600 rounded-xl px-3 py-2">
            ...
            <input type="text" value={search} onChange={e => { setSearch(e.target.value); setPage(1) }} placeholder="Search by worker name..."
      ...
      <div className="bg-white dark:bg-slate-800 rounded-2xl shadow-sm border border-slate-100 dark:border-slate-700 overflow-hidden">
            ...
            <tbody className="divide-y divide-slate-50 dark:divide-slate-700">
              ...
              {paginated.map(worker => (
                <tr key={worker.id} className="hover:bg-slate-50 dark:hover:bg-slate-700/30 transition-colors">
                  ...
                  <td className="px-5 py-3.5">
                    <span className={`px-2.5 py-1 rounded-full text-xs font-medium ${RISK_BADGE_CLASSES[worker.risk_level] || 'bg-slate-100 text-slate-600 dark:bg-slate-700 dark:text-slate-300'}`}>{worker.risk_level}</span>
                  </td>
                  <td className="px-5 py-3.5 text-slate-500 dark:text-slate-400">{formatDate(worker.last_checkup_date)}</td>
                  <td className="px-5 py-3.5">
                    <Link to={`/admin/worker/${worker.id}`} className="text-indigo-600 dark:text-indigo-400 text-sm font-medium hover:underline">View details</Link>
                  </td>
                </tr>
              ))}
            </tbody>
        ...
        <div className="flex items-center justify-between px-5 py-4 border-t border-slate-100 dark:border-slate-700">
          <p className="text-sm text-slate-500 dark:text-slate-400">
            {filtered.length === 0 ? '0 workers' : `${(page - 1) * PAGE_SIZE + 1}–${Math.min(page * PAGE_SIZE, filtered.length)} of ${filtered.length}`}
          </p>
